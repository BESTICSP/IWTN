...
#####################
# content encoder
        content_encoder = [nn.ReflectionPad2d(3),
        ...
        for i in range(n_downsampling):  # add downsampling layers
            mult = 2 ** i
            if no_antialias:
                content_encoder += [nn.Conv2d(ngf * mult, ngf * mult * 2, kernel_size=3, stride=2, padding=1, bias=use_bias),
                          norm_layer(ngf * mult * 2),
                          nn.ReLU(True)]
            else:
                content_encoder += [nn.Conv2d(ngf * mult, ngf * mult * 2, kernel_size=3, stride=1, padding=1, bias=use_bias),
                          norm_layer(ngf * mult * 2),
                          nn.ReLU(True),
...
# Self attention的尝试
        n_transblocks = 2
        selfattn = []
        for i in range(n_transblocks):
            selfattn += [SelfAttentionEncoder()]
...
####################
        ...
        self.enc = nn.Sequential(*content_encoder)
        # self.trans = nn.Sequential(*transformer)
        self.attn = nn.Sequential(*selfattn)
        ...
        if len(layers) > 0:
            ...
            for layer_id, layer in enumerate(self.model):
                ...
                if layer_id == layers[-1] and encode_only:
                    return None, feats
            # aggregated_feat, aggregated_box = self.trans(feat, x['A_box'], 20)
            # fake = self.dec(aggregated_feat)
            return feat, feats
        else:
            # 在前向传播的时候，走的是这一条通道
            x = self.enc(x) # x.shape [4,256,64,64]
            """
                改成self attn的transblock
            """
            #print('x',s.shape)
            x = self.attn(x)
            #print(x.shape)
            ...
            out = self.dec(x) # out.shape [4,3,256,256]


            return out,None

        # rand_style = torch.randn(x.shape[0],8,1,1)
        # losses.assign_adain_params()
...
class ResnetBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        return out
# 写一个用Self-attn的encoder
class SelfAttentionEncoder(nn.Module):
    def __init__(self,dim=256,hidden_dim=256):
        super(SelfAttentionEncoder, self).__init__()
        self.layernorm = nn.LayerNorm(normalized_shape=64)
        self.attn = Self_Attn(in_dim=dim,activation='relu')
        self.ffn = FeedForward(dim=dim,hidden_dim=hidden_dim)

    def forward(self,x):

        x = self.layernorm(x)
        attn_score, attn_feat = self.attn(x)
        x = torch.mul(x, attn_score) + x

        #
        xf = self.layernorm(x) # ([4, 256, 64, 64])
        xf = self.ffn(xf)
        xf = xf + x
        #print('xxxf', xf.shape)
        return x
...
class FeedForward(nn.Module):
    ...
    def forward(self, x):
        x = x.view(4,256,4096).permute(0,2,1)
        x = self.net(x)
        x = x.permute(0,1,2).view(4,256,64,64)
        return x
